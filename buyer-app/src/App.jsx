...
function getTrackingTimeline(order) {
  ...
  return STATUS_SEQUENCE.map((status, index) => {
    const estimate = addDays(createdAt, STATUS_DAY_OFFSET[status] || 0);
    return {
      status,
...
function BuyerAppShell() {
  ...
  const onOrderNow = useCallback(async (product) => {
    try {
      await api.placeOrder([{ productId: product._id, quantity: 1 }]);
      await refreshAll();
      setScreen("orders");
            ...
            {displayedOrders.length ? displayedOrders.map((order) => (
              <article key={order._id} className="order-card">
                ...
                <div className="comment-section">
                  ...
                  <button type="button" className="btn" style={{ background: "#fee2e2", marginTop: 10 }} onClick={() => removeOrder(order._id)}>Remove Order</button>
                </div>
              </article>
